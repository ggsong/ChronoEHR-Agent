#!/usr/bin/env python3
"""Post-hoc calibration for Random Forest baseline predictions."""
...
def load_rf_predictions(project_root: Path) -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Missing Random Forest predictions: {path}")
...
def write_report(performance: pd.DataFrame, report_path: Path, sklearn_version: str) -> None:
    ...
    lines = [
        "# Calibrated Random Forest Baseline Report",
        "",
    ...
    lines.extend(
        [
            ...
            "- Platt calibration is monotonic, so AUROC/AUPRC usually stay close to the uncalibrated RF.",
            "- Isotonic calibration can be more flexible but may overfit if validation data are limited.",
        ]
    )
...
def main() -> None:
    ...
    rf = load_rf_predictions(args.project_root)
    rows = []
    prediction_parts = []
    for study, group in rf.groupby("study", sort=False):
        ...
        for method, calibrator in calibrators.items():
            for split_name, split_df in [("validation", validation), ("test", test)]:
                ...
                metric_row.update(
                    {
                        ...
                        "model": f"calibrated_random_forest_{method}",
                ...
                pred["model"] = f"calibrated_random_forest_{method}"
    ...
    performance.to_csv(tables / "calibrated_random_forest_performance.csv", index=False)
    predictions.to_csv(tables / "calibrated_random_forest_predictions.csv", index=False)
    write_report(performance, reports / "calibrated_random_forest_report.md", detail)

    print("Calibrated Random Forest baseline complete")
