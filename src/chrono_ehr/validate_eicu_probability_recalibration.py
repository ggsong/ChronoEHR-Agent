#!/usr/bin/env python3
"""Validate eICU probability recalibration outputs."""
...
def validate(project_root: Path) -> pd.DataFrame:
    ...
    report_path = project_root / "outputs" / "reports" / "eicu_probability_recalibration_report.md"
    paths = {
        "predictions": table_dir / "eicu_probability_recalibration_predictions.csv",
        "metrics": table_dir / "eicu_probability_recalibration_metrics.csv",
        "deciles": table_dir / "eicu_probability_recalibration_deciles.csv",
        "summary": table_dir / "eicu_probability_recalibration_summary.csv",
        "decision": table_dir / "eicu_probability_recalibration_decision_curve.csv",
        "baseline_predictions": table_dir / "eicu_first24h_logistic_baseline_predictions.csv",
    }
    ...
    if "raw" in test_metrics.index:
        ...
        best_brier = float(test_metrics.loc[[method for method in EXPECTED_METHODS if method != "raw"], "Brier_score"].min())
        rows.append(row("test_brier_improves_after_recalibration", "PASS" if best_brier < raw_brier else "FAIL", str(paths["metrics"]), f"raw={raw_brier:.6f}, best_calibrated={best_brier:.6f}"))
        raw_error = float(test_metrics.loc["raw", "absolute_mean_error"])
        best_error = float(test_metrics.loc[[method for method in EXPECTED_METHODS if method != "raw"], "absolute_mean_error"].min())
...
def main() -> None:
    ...
    table_path = args.project_root / "outputs" / "tables" / "eicu_probability_recalibration_validation.csv"
    report_path = args.project_root / "outputs" / "reports" / "eicu_probability_recalibration_validation.md"
    table_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.parent.mkdir(parents=True, exist_ok=True)
    checks.to_csv(table_path, index=False)
    report_path.write_text(
        f"""# eICU Probability Recalibration Validation
...
""",
    ...
    print(f"eICU probability recalibration validation checks: {len(checks)}")
