#!/usr/bin/env python3
"""Validate CHARLS incident diabetes model-comparison outputs."""
...
def validate(project_root: Path) -> pd.DataFrame:
    metrics_path = project_root / "outputs" / "tables" / "charls_incident_diabetes_model_comparison_metrics.csv"
    predictions_path = project_root / "outputs" / "tables" / "charls_incident_diabetes_model_comparison_predictions.csv"
    importances_path = project_root / "outputs" / "tables" / "charls_incident_diabetes_model_comparison_importances.csv"
    ...
    if not importances.empty and "feature" in importances:
        bad_features = [
            ...
            if feature != "not_available" and (not feature.startswith("charls_baseline_") or any(token in feature.lower() for token in FORBIDDEN_FEATURE_TOKENS))
        ]
        rows.append(row("importance_features_baseline_only", "PASS" if not bad_features else "FAIL", str(importances_path), "bad=" + ",".join(bad_features[:20])))
...
def main() -> None:
    ...
    table_path = args.project_root / "outputs" / "tables" / "charls_incident_diabetes_model_comparison_validation.csv"
    report_path = args.project_root / "outputs" / "reports" / "charls_incident_diabetes_model_comparison_validation.md"
    table_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.parent.mkdir(parents=True, exist_ok=True)
    checks.to_csv(table_path, index=False)
    report_path.write_text(
        f"""# CHARLS Incident Diabetes Model Comparison Validation
...
""",
    ...
    print(f"CHARLS model comparison validation checks: {len(checks)}")
