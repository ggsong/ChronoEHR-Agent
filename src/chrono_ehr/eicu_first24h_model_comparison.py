#!/usr/bin/env python3
"""Run eICU first-24h traditional model comparisons."""
...
def predict_split(
    ...
    predictions = frame[["stay_id", "patient_id", "split", LABEL_COLUMN]].copy()
...
def write_report(project_root: Path, metrics: pd.DataFrame, importances: pd.DataFrame) -> Path:
    ...
    report_path.write_text(
        f"""# eICU First-24h Model Comparison
...
- Split policy: reuse the existing eICU train/validation/test split.
- Leakage gate: run before modeling; outcome/discharge/future-offset features remain blocked.
...
This comparison tests whether nonlinear traditional models improve ranking over the logistic first-24h baseline on the same leakage-gated eICU slice. Absolute risk should be interpreted with calibration and decision-curve checks.
""",
        encoding="utf-8",
    )
    return report_path
...
def main() -> None:
    ...
    metrics_df.to_csv(tables / "eicu_first24h_model_comparison_metrics.csv", index=False)
    predictions_df.to_csv(tables / "eicu_first24h_model_comparison_predictions.csv", index=False)
    importances_df.to_csv(tables / "eicu_first24h_model_comparison_importances.csv", index=False)
