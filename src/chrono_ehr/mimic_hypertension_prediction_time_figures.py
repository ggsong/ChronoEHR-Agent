#!/usr/bin/env python3
"""Generate hypertension prediction-time comparison figures."""
...
def plot_metric(df: pd.DataFrame, metric: str, out_path: Path) -> None:
    ...
    plt.ylabel(metric)
    plt.title(f"Hypertension prediction-time comparison: {metric}")
...
def write_report(paths: dict[str, Path], output: Path) -> None:
    text = f"""# MIMIC Hypertension Prediction-Time Figure Report

已生成 hypertension prediction-time model comparison 图：

- AUROC bar chart: `{paths["auroc"]}`
- AUPRC bar chart: `{paths["auprc"]}`

这两张图用于展示高血压 30 天再入院预测中，入院时、入院后 24 小时、出院时三个时间点的模型表现变化。
"""
...
def main() -> None:
    ...
    df = pd.read_csv(tables / "mimic_hypertension_prediction_time_model_performance.csv")
    paths = {
        "auroc": figures / "mimic_hypertension_prediction_time_auroc.png",
        "auprc": figures / "mimic_hypertension_prediction_time_auprc.png",
    }
    plot_metric(df, "AUROC", paths["auroc"])
    plot_metric(df, "AUPRC", paths["auprc"])
    write_report(paths, reports / "mimic_hypertension_prediction_time_figure_report.md")
