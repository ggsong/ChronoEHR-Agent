#!/usr/bin/env python3
"""Run heart failure prediction-time logistic baselines."""
...
def main() -> None:
    args = parse_args()
    config = load_prediction_time_config("heart_failure")
    ...
    print("MIMIC heart failure prediction-time models complete")
    for row in performance[performance["split"].eq("test")].itertuples(index=False):
        print(f"{row.feature_set}: AUROC={row.AUROC:.4f} AUPRC={row.AUPRC:.4f} Brier={row.Brier_score:.4f}")
