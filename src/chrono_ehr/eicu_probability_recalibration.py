#!/usr/bin/env python3
"""Recalibrate eICU first-24h mortality baseline probabilities."""
...
def load_predictions(project_root: Path) -> pd.DataFrame:
    path = project_root / "outputs" / "tables" / "eicu_first24h_logistic_baseline_predictions.csv"
    if not path.exists():
        raise FileNotFoundError(f"Missing eICU first-24h predictions: {path}")
    df = pd.read_csv(path)
    required = {"stay_id", "patient_id", "split", LABEL_COLUMN, "predicted_risk", "prediction_time", "feature_set", "model"}
    missing = sorted(required - set(df.columns))
    if missing:
        raise ValueError(f"eICU prediction file missing columns: {missing}")
    return df
...
def fit_calibrators(predictions: pd.DataFrame) -> dict[str, object]:
    ...
    if validation.empty:
        raise ValueError("Validation split is required for eICU probability recalibration.")
...
def apply_calibration(predictions: pd.DataFrame, calibrators: dict[str, object]) -> pd.DataFrame:
    ...
    for method, values in calibrated.items():
        part = predictions[["stay_id", "patient_id", "split", LABEL_COLUMN, "prediction_time", "feature_set", "model"]].copy()
...
def write_report(project_root: Path, metrics: pd.DataFrame, summary: pd.DataFrame, decision: pd.DataFrame, calibrators: dict[str, object]) -> Path:
    report = project_root / "outputs" / "reports" / "eicu_probability_recalibration_report.md"
    report.parent.mkdir(parents=True, exist_ok=True)
    test_metrics = metrics[metrics["split"].astype(str).eq("test")]
    test_summary = summary[summary["split"].astype(str).eq("test")]
    test_decision = decision[decision["split"].astype(str).eq("test")]
    report.write_text(
        f"""# eICU Probability Recalibration

- Boundary: research model evaluation only; no medical QA, diagnosis, or treatment recommendation.
- Source model: balanced logistic regression for eICU first-24h hospital mortality.
...
def main() -> None:
    ...
    predictions.to_csv(tables / "eicu_probability_recalibration_predictions.csv", index=False)
    metrics.to_csv(tables / "eicu_probability_recalibration_metrics.csv", index=False)
    deciles.to_csv(tables / "eicu_probability_recalibration_deciles.csv", index=False)
    summary.to_csv(tables / "eicu_probability_recalibration_summary.csv", index=False)
    decision.to_csv(tables / "eicu_probability_recalibration_decision_curve.csv", index=False)
