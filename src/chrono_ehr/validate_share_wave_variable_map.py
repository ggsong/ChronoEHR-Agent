#!/usr/bin/env python3
"""Validate the SHARE wave-variable map."""
...
REQUIRED_CONCEPTS = {
    ...
    "followup_wave2_diabetes_diagnosis",
    "followup_wave4_diabetes_diagnosis",
}
...
def validate(project_root: Path) -> pd.DataFrame:
    map_path = project_root / "outputs" / "tables" / "share_wave_variable_map.csv"
    inv_path = project_root / "outputs" / "tables" / "share_harmonized_variable_inventory.csv"
    report_path = project_root / "outputs" / "reports" / "share_wave_variable_map.md"
    ...
    future_wave = wave_map["wave"].astype(str).str.contains("wave2|wave4|wave5|wave6|wave7|wave8", regex=True, na=False)
    ...
    future_baseline = baseline_features[baseline_features["wave"].astype(str).str.contains("wave2|wave4|wave5|wave6|wave7|wave8", regex=True, na=False)]
...
def main() -> None:
    ...
    table_path = args.project_root / "outputs" / "tables" / "share_wave_variable_map_validation.csv"
    report_path = args.project_root / "outputs" / "reports" / "share_wave_variable_map_validation.md"
    table_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.parent.mkdir(parents=True, exist_ok=True)
    checks.to_csv(table_path, index=False)
    report_path.write_text(
        f"""# SHARE Wave Variable Map Validation
...
""",
    ...
    print(f"SHARE wave-map validation checks: {len(checks)}")
