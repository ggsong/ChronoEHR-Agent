#!/usr/bin/env python3
"""Extract hypertension first-24h and discharge-time lab features."""
...
from mimic_diabetes_lab_features import TARGET_ITEMIDS as DIABETES_ITEMIDS
from mimic_diabetes_lab_features import empty_state, update_state


LABS = ["creatinine", "bun", "potassium", "sodium", "bicarbonate", "hemoglobin", "glucose", "albumin"]


def target_itemids() -> dict[int, str]:
    mapping: dict[int, str] = {}
    for lab in ["creatinine", "bun", "potassium", "sodium", "bicarbonate", "hemoglobin", "albumin"]:
        for itemid in FIRST_PASS_ITEMIDS[lab]:
            mapping[int(itemid)] = lab
    for itemid, lab in DIABETES_ITEMIDS.items():
        if lab == "glucose":
            mapping[int(itemid)] = "glucose"
    return mapping


def load_cohort(project_root: Path, window_spec: dict | None = None) -> pd.DataFrame:
    path = project_root / "data" / "processed" / "mimic_hypertension_readmission_cohort.csv"
...
def process_labevents(
    ...
    features_24h = state_to_features(state_24h, cohort_hadm, "htnlab24h")
    features_discharge = state_to_features(state_discharge, cohort_hadm, "htnlabdischarge")
    availability = pd.concat(
        [
            summarize_availability(features_24h, "htnlab24h", "first_24h"),
            summarize_availability(features_discharge, "htnlabdischarge", "admission_to_discharge"),
        ],
        ignore_index=True,
    )
...
def write_report(stats: dict[str, int], availability: pd.DataFrame, report_path: Path) -> None:
    ...
    text = f"""# MIMIC Hypertension Lab Feature Extraction Report
...
第一版高血压 lab features 使用覆盖率较稳定、与慢病住院风险相关的常规化验：creatinine、BUN、potassium、sodium、bicarbonate、hemoglobin、glucose、albumin。
...
- hypertension cohort HADM rows: {stats["cohort_hadm_rows"]}
...
The first-24h feature file is safe for an in-hospital 24-hour prediction time. The discharge feature file is safe only for discharge-time prediction and must not be reused for admission-time prediction.
"""
...
def main() -> None:
    ...
    features_24h.to_csv(processed / "mimic_hypertension_lab_features_24h.csv", index=False)
    features_discharge.to_csv(processed / "mimic_hypertension_lab_features_discharge.csv", index=False)
    availability.to_csv(tables / "mimic_hypertension_lab_feature_availability.csv", index=False)
    pd.DataFrame([stats]).to_csv(tables / "mimic_hypertension_lab_extraction_stats.csv", index=False)
    write_report(stats, availability, reports / "mimic_hypertension_lab_feature_report.md")

    print("MIMIC hypertension lab features extracted")
