#!/usr/bin/env python3
"""Extract heart-failure first-24h and discharge-time lab features."""
...
def target_itemids() -> dict[int, str]:
    ...
    mapping.update(HEART_FAILURE_ITEMIDS)
    return mapping


def load_cohort(project_root: Path, window_spec: dict | None = None) -> pd.DataFrame:
    path = project_root / "data" / "processed" / "mimic_heart_failure_readmission_cohort.csv"
...
def process_labevents(
    ...
    features_24h = state_to_features(state_24h, cohort_hadm, "hflab24h")
    features_discharge = state_to_features(state_discharge, cohort_hadm, "hflabdischarge")
    availability = pd.concat(
        [
            summarize_availability(features_24h, "hflab24h", "first_24h"),
            summarize_availability(features_discharge, "hflabdischarge", "admission_to_discharge"),
        ],
        ignore_index=True,
    )
...
def write_report(stats: dict[str, int], availability: pd.DataFrame, report_path: Path) -> None:
    ...
    text = f"""# MIMIC Heart Failure Lab Feature Extraction Report
...
- Kidney/electrolyte/general severity markers: creatinine, BUN, potassium, sodium, bicarbonate, hemoglobin, albumin.
- Heart-failure-relevant markers available in MIMIC-IV lab dictionary: NT-proBNP and troponin.
...
- heart failure cohort HADM rows: {stats["cohort_hadm_rows"]}
...
def main() -> None:
    ...
    features_24h.to_csv(processed / "mimic_heart_failure_lab_features_24h.csv", index=False)
    features_discharge.to_csv(processed / "mimic_heart_failure_lab_features_discharge.csv", index=False)
    availability.to_csv(tables / "mimic_heart_failure_lab_feature_availability.csv", index=False)
    pd.DataFrame([stats]).to_csv(tables / "mimic_heart_failure_lab_extraction_stats.csv", index=False)
    write_report(stats, availability, reports / "mimic_heart_failure_lab_feature_report.md")

    print("MIMIC heart failure lab features extracted")
