#!/usr/bin/env python3
"""Validate the SHARE incident diabetes cohort skeleton."""
...
def validate(project_root: Path) -> pd.DataFrame:
    cohort_path = project_root / "data" / "processed" / "share_incident_diabetes_cohort_skeleton.csv"
    summary_path = project_root / "outputs" / "tables" / "share_incident_diabetes_cohort_summary.csv"
    wave_summary_path = project_root / "outputs" / "tables" / "share_incident_diabetes_wave_outcome_summary.csv"
    report_path = project_root / "outputs" / "reports" / "share_incident_diabetes_cohort_skeleton.md"
    ...
    rows.append(row("baseline_age_min_50", "PASS" if cohort["baseline_age_years"].min() >= 50 else "FAIL", str(cohort_path), f"min_age={cohort['baseline_age_years'].min()}"))
    ...
    label_values = sorted(cohort["incident_diabetes_wave2_or_wave4"].dropna().unique().tolist())
    ...
    followup_known = cohort["followup_wave2_diabetes_known"].astype(bool) | cohort["followup_wave4_diabetes_known"].astype(bool)
    ...
    forbidden = {"r2diabe", "r4diabe", "r2rxdiab", "r4rxdiab", "inw2", "inw4"}
...
def main() -> None:
    ...
    table_path = args.project_root / "outputs" / "tables" / "share_incident_diabetes_cohort_validation.csv"
    report_path = args.project_root / "outputs" / "reports" / "share_incident_diabetes_cohort_validation.md"
    table_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.parent.mkdir(parents=True, exist_ok=True)
    checks.to_csv(table_path, index=False)
    report_path.write_text(
        f"""# SHARE Incident Diabetes Cohort Validation
...
""",
    ...
    print(f"SHARE cohort validation checks: {len(checks)}")
